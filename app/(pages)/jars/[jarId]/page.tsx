        ...
        return (
            <main className="space-y-6 p-6">
                ...
                <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <MetricCard label="Current balance" value={currencyFormatter.format(overview.balance)} />
                    <MetricCard label="Total income" value={currencyFormatter.format(overview.incomeTotal)} />
                    <MetricCard
                        ...
                        {overview.goals.length ? (
                            <div className="space-y-4">
                                {overview.goals.map((goal) => {
                                    const formattedTarget = currencyFormatter.format(goal.targetAmount);
                                    const formattedCurrent = currencyFormatter.format(goal.currentAmount);
                                    ...
                                    return (
                                        <div key={goal.id} className="space-y-2 rounded-lg border p-4">
                                            ...
                                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                                                ...
                                                {goal.targetDate ? (
                                                    <span>Target {dateFormatter.format(new Date(goal.targetDate))}</span>
                                                ) : null}
                                            </div>
                                        </div>
                                    );
